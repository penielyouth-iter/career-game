...
function normalizeCellsJson(value:unknown):BoardCell[]{
 ...
 return value.map((cell,index)=>{
  ...
  if(item.choice==="charity"||item.choice==="support"||item.choice==="teamGrowth")next.choice=item.choice;
  return next;
 }).sort((a,b)=>a.id-b.id);
}
