...
export default function AdminApp(){
 const[auth,setAuth]=useState<boolean|null>(null);const[password,setPassword]=useState("");const[config,setConfig]=useState<GameConfig|null>(null);const[game,setGame]=useState<GameState|null>(null);
 ...
 useEffect(()=>{Promise.all([fetch("/api/admin/session").then(r=>r.json()),fetch("/api/config").then(r=>r.json()),fetch("/api/game",{cache:"no-store"}).then(r=>r.json())]).then(([a,c,g])=>{const nextConfig=c.config as GameConfig;setAuth(a.authenticated);setConfig(nextConfig);setGame(g.state);const first=nextConfig.characters[0]?.id||"";setSelectedRole(first);setJsonText(first?cleanJson(nextConfig.characters[0]):"")})},[]);
 ...
 const publish=async()=>{setMsg("發布中…");const r=await fetch("/api/config",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({config})});setMsg(r.ok?"已發布！新設定立即套用到目前遊戲。":"發布失敗，請重新登入。");if(r.ok){const data=await r.json();setConfig(data.config)}};
 const reset=async()=>{if(!confirm("確定要清除目前遊戲進度？卡片、角色與格子設定不會被刪除。"))return;const r=await fetch("/api/admin/reset",{method:"POST"});setMsg(r.ok?"遊戲已重置。":"重置失敗。")};
 ...
 const adjustStat=async(playerId:string,key:StatKey,delta:number)=>{if(!game||adjusting)return;const current=game.players[playerId]?.stats[key]??0;const next=Math.max(0,current+delta);setAdjusting(playerId+key);try{const r=await fetch("/api/admin/game",{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({playerId,stats:{[key]:next}})});const data=await r.json();if(!r.ok)throw 0;setGame(data.state);setMsg("已即時調整角色數值。")}catch{setMsg("能力調整失敗，請重新登入。")}finally{setAdjusting("")}};
